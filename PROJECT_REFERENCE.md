...
| `ghl-update-opportunity`      | Updates an opportunity's `pipelineStageId` + optional `status` in GHL via `PUT /opportunities/:id`, then mirrors the change into `public.opportunities` so the UI reflects it before the next sync                               | Admin / RD / VP / manager / sales_rep / franchisee with `can_access_ghl_location(...)` |
