...
| `pipeline_stages`                           | Stages discovered from GHL, with `counts_as_show / counts_as_no_show / counts_as_sale / counts_as_trial` flags. Admin-set flag values are preserved on re-sync (heuristic defaults only apply to newly-discovered stages) | Via `can_access_ghl_location()`; admin writes via `pipeline_stages_admin_write` |
...
| `ghl-set-tag`                 | Adds or removes a tag on a GHL contact via `POST` / `DELETE /contacts/:id/tags`. Currently used by the AI Off toggle on LeadDetail (applies the `ai off` tag that Closebot's workflow watches for to pause automation). Mirrors the change into `public.contacts.tags` so the UI reflects immediately without waiting for the next sync. Extensible to any future tag workflow               | Admin / RD / VP / manager / sales_rep / franchisee with `can_access_ghl_location(...)` |
...
> **As of April 21, 2026, ADPS is calculated from real `opportunities.mrr` entries per sale**; falls back to the $220 Aira benchmark when a location has <5 real-value sales in the period (the Triage card shows a "Using Aira benchmark (sample too small)" pill). Sales are captured as two distinct dollar values via the Sale modal that fires on any stage → `counts_as_sale` transition: `gross_sale` (all cash at close — enrollment + first month + PIF if any) and `mrr` (monthly recurring membership rate). `mrr` is also mirrored into GHL's `monetaryValue` so GHL's own UI stays meaningful. Triage `floor_gap_dollars` is now annualized: `(floor − actual) × (ADPS × 12 + avg_gross_per_pif × pif_attach_rate)` using real values where available, Aira benchmarks ($220 ADPS, 20% PIF attach, $997 avg PIF) otherwise.
> | Attrition rate | ≤7% | — |
...
### Trial Passes (separate metric lane)

Trial passes (7 Day Pass) are tracked separately from direct sales. The Trial Passes KPI card surfaces Given / Converted / Not Converted / Active counts per period. When a lead moves from "7 Day Pass" to "7 Day Pass Converted", the stage has `counts_as_sale = true` and the Sale modal fires to capture gross + MRR — so converted trials still roll into total sales and MRR. The Rep Leaderboard's TRIAL→SALE column shows how many of each rep's sales came via trial conversion, surfacing which reps work the long-game trial system vs. first-visit closes.
...
**April 20, 2026 — Sales captured as Gross + MRR, two distinct dollar values.**
When a portal user marks a lead Sold, the modal captures (a) **Gross Sale** — all cash collected at close including enrollment, first month, and PIF; (b) **MRR** — the monthly recurring membership rate going forward; (c) PIF flag. These are stored as separate columns (`gross_sale`, `mrr`, `is_pif`) on `opportunities`. The prior single-field design (`monetary_value` conflating all three concepts) hid the real operational signals — a rep can have high gross with low MRR (heavy PIF, discounted memberships) or vice versa, and Alyssa needs both numbers to coach. GHL's `monetaryValue` is kept in sync with `mrr` so GHL's own UI stays meaningful. Dollar math across the portal now uses real `mrr` where available, Aira's $220 benchmark otherwise, with the <5-sample noise guard intact.

**April 20, 2026 — Leaderboard tracks full rep activity, not just terminal sales.**
Team This Month on the single-location Dashboard + the new Rep Activity — Territory section on TerritoryDashboard both show Calls + Appts + Sales + Gross + MRR per rep. Reps need to see themselves on the board the moment they do work, not only when they close. Gaming risk (padding call counts) accepted — Alyssa sees call-per-sale ratios and can coach outliers. Sort defaults to MRR desc so heavy-activity-low-revenue reps surface at the bottom where they belong. Reps with zero activity across every column are hidden; the leaderboard is an activity view, not a roster. `rep_leaderboard` now takes `text[]` so the same RPC powers per-location and territory-aggregated renders; the Territory view folds (rep × location) rows into one rep row with a 📍 tag listing where the activity came from.

**April 20, 2026 — Trial passes are their own metric lane; converted trials still count as sales.**
Fox Lake added "7 Day Pass Converted" / "7 Day Pass Not Converted" / "Follow Up" stages alongside the existing "7 Day Pass" stage. The portal now surfaces trial counts as a distinct KPI card on the Dashboard (Given / Converted / Not Converted / Active, MTD) via a new `location_trial_counts` RPC; converted trials still roll into total sales + MRR because they ARE sales (the "7 Day Pass Converted" stage has `counts_as_sale = true`, so the Sale modal fires on transition to capture gross + MRR). New `counts_as_trial` column on `pipeline_stages` plus a TRIAL→SALE column added to `rep_leaderboard` lets VPs see which reps close via the trial system (long-game conversion) vs. which close directly at tour — both valid patterns, Alyssa coaches to each rep's strength.

**April 20, 2026 — "Show" means physically walked in, not booked.**
Corrected Fox Lake's flag state: "Booked Appt" and "Confirmed Appt" no longer set `counts_as_show = true`. A show is only counted when someone actually attends a tour (moves to "Sold", "Showed/Did Not Join", "7 Day Pass", "7 Day Pass Converted", or "7 Day Pass Not Converted"). This drops the historical show-rate number but makes every downstream coaching diagnosis honest — "shows broken" now means what it says. Also applied to `classifyStage()` in `sync-ghl-to-cache`: booked/confirmed stages no longer auto-flag as shows for newly-discovered stages; admins can still flip them on per-location via the Pipeline Stage Editor. Mishawaka's flags left untouched per the scope guardrail — Alyssa / Jasmine can adjust via the editor if they want the same correction there.

**April 20, 2026 — Pipeline stage flags are preserved on re-sync.**
Before today, `sync-ghl-to-cache` upserted pipeline stages with heuristic-classified flag values on every run, which meant an admin editing "Sold" to also count as a show in the Pipeline Stage Editor would see their change silently reverted on the next sync. Fixed: the sync now splits incoming GHL stages into (new, existing) by `ghl_stage_id` and writes two shapes — new stages get the full heuristic-default insert (now including `counts_as_trial`), existing stages get only `name` / `position` / `pipeline_id` updated. Admin-curated flags are untouchable by the sync. The sync also no longer exits early when a location already has `pipeline_id` set, so newly-added stages (like Fox Lake's 2026-04-20 additions) get picked up on the next run.

**April 20, 2026 — AI Off button routes through the portal, not GHL.**
Closebot occasionally goes off-rails on specific contacts (mis-reading intent, over-pestering, etc.). Operators can now tap "AI Off" on any lead — a new button in the LeadDetail action row — which adds the `ai off` tag via the new `ghl-set-tag` Edge Function. Closebot's existing GHL workflow watches for that tag and pauses automation when present. Tapping again removes the tag and re-enables. Button state is read from `contacts.tags` on mount and toggles optimistically (rollback on edge-function failure). `ghl-set-tag` is extensible to any future tag-based workflow. Role gating: any authenticated portal user with `can_access_ghl_location` can toggle — this is a coaching signal, not an admin action.

**April 20, 2026 — `admin-invite-user` re-links stranded auth accounts instead of refusing.**
When an admin adds a person whose email already has an `auth.users` row but no linked `public.users` row (typically a leftover from an earlier orphan-cleanup cycle — Jasmine Crago's April 19 seed hit this), the function now inserts the `public.users` + `user_locations` rows against the existing `auth_user_id` and sends a password-reset link instead of attempting a new invite. Without this the admin would hit Supabase's "A user with this email address has already been registered" 422 from `inviteUserByEmail` with no recourse short of opening SQL. Real duplicates (both `auth.users` AND `public.users` exist) still return a friendly 409 with `code: 'already_portal_user'` pointing the admin to the Edit flow instead. Rollback chain on the re-link path deliberately does NOT delete the auth record on failure — it preexisted, so leave it alone. The portal response carries `relinked: true/false` so the UI can show a distinct amber "info" toast ("Relinked existing account — they'll receive a password reset email") rather than a standard green success toast.

**April 20, 2026 — SMS sends write to cache immediately, don't wait for sync.**
`ghl-send-message` now updates `public.conversations` the moment a message leaves through GHL — appending to `full_thread`, bumping `last_message_at` / `last_message_type` / `last_message_body`, and inserting a fresh row for brand-new threads (when GHL returns a `conversationId`). The client does an optimistic append on Send, showing the bubble as "sending" during the round trip; on 2xx the thread reloads from the now-current cache and the optimistic row is replaced by the server copy, on error the bubble stays visible with a "↻ Tap to retry" affordance instead of being rolled back. Reasoning: without this, reps see their message go through GHL but not appear in the portal for up to a sync cycle — they'd either text again (duplicate sends) or open GHL to verify (architecture violation per the April 18 "portal is source of truth" decision). Cache writes on send are additive, not a replacement for `sync-ghl-to-cache` — that still runs as before and is authoritative for inbound messages and any outbound sent outside the portal. If the cache write fails for any reason we still return success to the client (GHL got the message) and log to `sync_log` with `sync_type='send_message_cache'` so the next sync pass can repair.

**April 20, 2026 — Text routes through GHL; Call stays on personal phones.**
The Text button on LeadDetail now opens an in-portal SMS composer that sends via `ghl-send-message` (GHL `POST /conversations/messages` with `type: "SMS"`). The Call button remains a `tel:` link to the rep's personal phone. Reasoning: outbound SMS from a rep's personal number bypasses GHL entirely — the conversation disappears from the lead's thread, inbound replies go to the rep's iMessage instead of the portal's Conversations view, and nobody else can pick up the handoff. Texting must be in-portal so the thread is shared team property. Calls are different: voice routing through GHL would cost ~$1,500/mo across the franchise and forces reps out of the phone app they already use (per earlier April 20 call-logging decision), so calls stay on personal phones and the portal captures outcomes via Log Call. After a send, the UI routes the rep to the Conversations view for that contact so they can watch for the reply in the same place they'd read inbound texts. Also: the Conversations list has a sticky search (Mishawaka has 478 cached threads, Fox Lake 132 — scrolling isn't viable).
